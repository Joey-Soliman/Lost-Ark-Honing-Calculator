...
def honing_calc(values, item_mats, items):
    ...
    # getting lists of mats required for each item
    armor_1302 = item_mats[0]
    armor_1340 = item_mats[1]
    weapon_1302 = item_mats[2]
    weapon_1340 = item_mats[3]
    ...
    if item == 1:
        ...
        for i in range(slvl, elvl):
            ...
            item_mats = item_totals(name, armor_silver_1302[i] * num, armor_gold_1302[i] * num,
                armor_shards_1302[i] * num, armor_fusion_1302[i] * num, 
    ...
    # armor 1340
        ...
        for i in range(slvl, elvl):
            ...
            item_mats = item_totals(name, armor_silver_1340[i] * num, armor_gold_1340[i] * num,
                armor_shards_1340[i] * num, 0, armor_fusion_1340[i] * num, 
    ...
    # weapon 1302
        ...
        for i in range(slvl, elvl):
            ...
            item_mats = item_totals(name, weapon_silver_1302[i] * num, weapon_gold_1302[i] * num,
                weapon_shards_1302[i] * num, weapon_fusion_1302[i] * num, 
    ...
    # weapon 1340
        ...
        for i in range(slvl, elvl):
            ...
            item_mats = item_totals(name, weapon_silver_1340[i] * num, weapon_gold_1340[i] * num,
                weapon_shards_1340[i] * num, 0, weapon_fusion_1340[i] * num, 
...
if __name__ == '__main__':   
    ...
    # defines what happens when reset is clicked
    def reset():
    ...
    # defines what happens when add is clicked
    def add():
        for i in my_game.get_children():
            my_game.delete(i)
        ...
        item_mats = [armor_1302, armor_1340, weapon_1302, weapon_1340]
        honing_calc(values, item_mats, item_list)
    ...
    # defines what happens when undo is selected
    def undo():
        for i in my_game.get_children():
            my_game.delete(i)
    ...
    # defintes what happens when redo is clicked
    def redo():
        global item_list
        global u_item_list
        global r_item_list
        ...
        u_item_list = copy.deepcopy(item_list)
        item_list = copy.deepcopy(r_item_list)
    ...
    # defines what happens when delete is clicked
    def delete():
        global item_list
        global u_item_list
    ...
    # defines what happens when an item is selected from the dropdown
    def item_select(e):
        global item
    ...
    # defines what happens when a starting level is selected from the dropdown
    def slvl_select(e):
        global slvl
        elvl = []
        item = item_entry.get()
    ...
    # defines what happens when the instructions button is clicked
    def instructions():
        instructions = Toplevel(ws)
        instructions.geometry("360x210")
        instructions.title("Instructions")
        Label(instructions, text="Step 1: Select item.").grid(row=0, column=0, sticky=W)
        Label(instructions, text="Step 2: Select starting level of item.").grid(row=1, column=0, sticky=W)
        Label(instructions, text="Step 3: Select desired ending level of item.").grid(row=2, column=0, sticky=W)
        Label(instructions, text="Step 4: Select desired number of items to include.").grid(row=3, column=0, sticky=W)
        Label(instructions, text="      - Armor can be 1-5. Weapon can only be 1.").grid(row=4, column=0, sticky=W)
        Label(instructions, text="Step 5: Click \'Add\'").grid(row=5, column=0, sticky=W)
        Label(instructions, text="Step 6: To delete an item, select the row and click \'Delete\'").grid(row=6, column=0, sticky=W)
        Label(instructions, text="Warning: The undo button can only go back one step.").grid(row=7, column=0, sticky=W)
        Label(instructions, text="Warning: Reset will remove all items.").grid(row=8, column=0, sticky=W)


    # defines what happens when the material guide button is clicked
    def material_guide():
        material_guide = Toplevel(ws)
        material_guide.geometry("570x260")
        material_guide.title("Material Aquisition Guide")
        Label(material_guide, text="Do in order based off what you like, but for the most part,").grid(row=0, column=0, sticky=W)
        Label(material_guide, text="guide is in order from most profitable to least.").grid(row=0, column=1, sticky=W)
        Label(material_guide, text="Weekly events are the most profitable:").grid(row=2, column=0, sticky=W)
        Label(material_guide, text="    - Abyss Raids and Legion Raids").grid(row=3, column=0, sticky=W)
        Label(material_guide, text="    - Ghost Ship").grid(row=4, column=0, sticky=W)
        Label(material_guide, text="    - Purchase from guild, PVP, pirate ship").grid(row=5, column=0, sticky=W)
        Label(material_guide, text="    - Purchase from event shop if available").grid(row=6, column=0, sticky=W)
        Label(material_guide, text="    - Challenge Guardian Raids").grid(row=7, column=0, sticky=W)
        Label(material_guide, text="Daily activiites:").grid(row=8, column=0, sticky=W)
        Label(material_guide, text="    - Field Bosses").grid(row=9, column=0, sticky=W)
        Label(material_guide, text="    - Chaos Dungeons").grid(row=10, column=0, sticky=W)
        Label(material_guide, text="    - Guardian Raids").grid(row=11, column=0, sticky=W)
        Label(material_guide, text="    - Una Dailies").grid(row=12, column=0, sticky=W)


    # initialize gui
    ws  = Tk()
    ws.title('Honing Calculator')
    ws.geometry('1100x950')
    ...
    Label(ws, text="Don't know where to start?").grid(row=0, column=0, sticky=NSEW)
    instructions_button = Button(ws, text="Instructions", command=instructions)
    instructions_button.grid(row=0, column=1, sticky=NSEW)

    Label(ws, text="Don't know where to gather materials?").grid(row=0, column=2, sticky=NSEW)
    material_guide_button = Button(ws, text="Material Acquisition Guide", command=material_guide)
    material_guide_button.grid(row=0, column=3, sticky=NSEW)

    item_entry_label = Label(ws, text="Item")
    item_entry_label.grid(row=1, column=0, sticky=NSEW)
    item_entry = ttk.Combobox(ws, value=item_options)
    item_entry.current(0)
    item_entry.grid(row=1, column=1, sticky=NSEW)
    item_entry.bind("<<ComboboxSelected>>", item_select)

    slvl_entry_label = Label(ws, text="Starting Level")
    slvl_entry_label.grid(row=1, column=2, sticky=NSEW)
    slvl_entry = ttk.Combobox(ws, value=[" "])
    slvl_entry.current(0)
    slvl_entry.grid(row=1, column =3, sticky=NSEW)
    slvl_entry.bind("<<ComboboxSelected>>", slvl_select)

    elvl_entry_label = Label(ws, text="End Level")
    elvl_entry_label.grid(row=1, column=4, sticky=NSEW)
    elvl_entry = ttk.Combobox(ws, value=[" "])
    elvl_entry.current(0)
    elvl_entry.grid(row=1, column =5, sticky=NSEW)

    count_entry_label = Label(ws, text="Count")
    count_entry_label.grid(row=1, column=6, sticky=NSEW)
    count_entry = ttk.Combobox(ws, value=[" "])
    count_entry.current(0)
    count_entry.grid(row=1, column =7, sticky=NSEW)

    add_button = Button(ws, text="Add", command=add)
    add_button.grid(row=1, column=8, sticky=NSEW)

    undo_button = Button(ws, text="Undo", command=undo, state=DISABLED)
    undo_button.grid(row=2, column=0, sticky=NSEW)

    redo_button = Button(ws, text="Redo", command=redo, state=DISABLED)
    redo_button.grid(row=2, column=1, sticky=NSEW)
    ...
    delete_button.grid(row=2, column=3, sticky=NSEW)
